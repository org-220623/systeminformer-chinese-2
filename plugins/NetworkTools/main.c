    ...
    )
{
    ...
    PhInsertEMenuItem(networkToolsMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MAINMENU_ACTION_TRACERT, L"跟踪路由地址(&T)...", NULL), ULONG_MAX);
    PhInsertEMenuItem(networkToolsMenu, PhPluginCreateEMenuItem(PluginInstance, 0, MAINMENU_ACTION_WHOIS, L"Whois 地址(&W)...", NULL), ULONG_MAX);
    ...
    )
{
    ...
    PhInsertEMenuItem(menuInfo->Menu, traceMenu = PhPluginCreateEMenuItem(PluginInstance, 0, NETWORK_ACTION_TRACEROUTE, L"跟踪路由(&T)", networkItem), 1);
    PhInsertEMenuItem(menuInfo->Menu, whoisMenu = PhPluginCreateEMenuItem(PluginInstance, 0, NETWORK_ACTION_WHOIS, L"Whois(&W)", networkItem), 2);
    ...
    case NETWORK_COLUMN_ID_BYTES_IN:
        {
            ...
            if (!NetworkExtensionEnabled && !Extension->BytesIn && PhGetOwnTokenAttributes().Elevated)
                PhMoveReference(&Extension->BytesIn, PhCreateString(L"扩展 TCP 统计信息已禁用"));
        }
        break;
    case NETWORK_COLUMN_ID_BYTES_OUT:
        {
            ...
            if (!NetworkExtensionEnabled && !Extension->BytesOut && PhGetOwnTokenAttributes().Elevated)
                PhMoveReference(&Extension->BytesOut, PhCreateString(L"扩展 TCP 统计信息已禁用"));
        }
        break;
    case NETWORK_COLUMN_ID_PACKETLOSS:
        {
            ...
            if (!NetworkExtensionEnabled && !Extension->LossText && PhGetOwnTokenAttributes().Elevated)
                PhMoveReference(&Extension->LossText, PhCreateString(L"扩展 TCP 统计信息已禁用"));
        }
        break;
    case NETWORK_COLUMN_ID_JITTER:
        {
            ...
            if (!NetworkExtensionEnabled && !Extension->JitterText && PhGetOwnTokenAttributes().Elevated)
                PhMoveReference(&Extension->JitterText, PhCreateString(L"扩展 TCP 统计信息已禁用"));
        }
        break;
    case NETWORK_COLUMN_ID_LATENCY:
        {
            ...
            if (!NetworkExtensionEnabled && !Extension->LatencyText && PhGetOwnTokenAttributes().Elevated)
                PhMoveReference(&Extension->LatencyText, PhCreateString(L"扩展 TCP 统计信息已禁用"));
        }
        break;
    }
}
    ...
    case DLL_PROCESS_ATTACH:
        {
            ...
            info->Description = L"提供网络连接的 Ping、跟踪路由和 Whois 查询。";
