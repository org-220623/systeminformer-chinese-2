    ...
    )
{
    ...
    config.pszContent = PhaFormatString(
        ...
        (startTimeRelativeString && startTimeString) ? PhaFormatString(L"%s 以前 (%s)", PhGetString(startTimeRelativeString), PhGetString(startTimeString))->Buffer : L"N/A"
        )->Buffer;
    ...
    )
{
    ...
    if (Context->Message) config.pszContent = PhaConcatStrings2(Context->Message, L"您确定要继续吗?")->Buffer;
