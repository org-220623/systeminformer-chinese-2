    ...
    )
{
...

    {
        ...
        if (Message) config.pszContent = PhaConcatStrings2(Message, L" 您确定要继续吗?")->Buffer;
    ...
    )
{
    ...
    {
        string = PhFormatString(L"%u %s", (ULONG)fortnights, (ULONG)fortnights == 1 ? L"个两星期" : L"个两星期");
    }
    else if (weeks >= 1)
    {
    ...
    )
{
    ...
    if (SHOpenFolderAndSelectItems_I && SHParseDisplayName_I)
    {
        ...
        {
            PhShowError2(WindowHandle, L"无法找到位置。", L"%s", FileName);
        }
    }
    else
    {
